...
from data_handler import get_data_images
...
def load_arguments():
    ...
    parser.add_argument('-s', '--size', type=int, default=32, help='Width x height number for resizing image (square)')
    parser_args = vars(parser.parse_args())
    return parser_args


def train(arguments):
    X_train, y_train, X_valid, y_valid, X_test, y_test = get_data_images(arguments['dataset'])
    ...
    # Init model
    model = ModelHumorPrediction("PredictingHumor", output_folder=output)
    model.init()
